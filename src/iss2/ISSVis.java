...
class ConstraintsChecker {
	...
	// Solar array names
	static final String[] SOLAR_ARRAY_NAME = new String[] { "1A", "2A", "3A",
			"4A", "1B", "2B", "3B", "4B" };
	...
	static final double[] MIN_POWER_REQ = new double[] { 6300.0, 6400.0,
			6700.0, 8900.0, 5500.0, 8100.0, 4400.0, 4300.0 };
		...
		for (int i = 0; i < data.length; i++)
			if (Double.isNaN(data[i])) {
				errors.add("NaN elements in return value are not "
						+ "allowed. Minute = " + t + ". Position (0-based) = "
						+ i + ".");
				return;
			}
		...
		for (int i = 0; i < angleSARJ[t].length; i++)
			if (angleSARJ[t][i] < 0.0 || angleSARJ[t][i] >= 360.0) {
				errors.add("Each SARJ angle needs to be from 0.0 "
						+ "(inclusive) to 360.0 (exclusive). Minute = " + t
						+ ". SARJ = " + SARJ_NAME[i] + ". Your angle = "
						+ angleSARJ[t][i] + ".");
			}
		...
		for (int i = 0; i < angleBGA[t].length; i++)
			if (angleBGA[t][i] < 0.0 || angleBGA[t][i] >= 360.0) {
				errors.add("Each BGA angle needs to be from 0.0 "
						+ "(inclusive) to 360.0 (exclusive). Minute = " + t
						+ ". BGA = " + SOLAR_ARRAY_NAME[i] + ". Your angle = "
						+ angleBGA[t][i] + ".");
			}
		...
		for (int i = 0; i < speedSARJ[t].length; i++)
			if (Math.abs(speedSARJ[t][i]) > MAX_SARJ_SPEED) {
				errors.add("The absolute value of SARJ rotation speed "
						+ "can't exceed " + MAX_SARJ_SPEED + ". Minute = " + t
						+ ". SARJ = " + SARJ_NAME[i] + ". Your speed = "
						+ speedSARJ[t][i] + ".");
			}
		...
		for (int i = 0; i < speedBGA[t].length; i++)
			if (Math.abs(speedBGA[t][i]) > MAX_BGA_SPEED) {
				errors.add("The absolute value of BGA rotation speed "
						+ " can't exceed " + MAX_BGA_SPEED + ". Minute = " + t
						+ ". BGA = " + SOLAR_ARRAY_NAME[i] + ". Your speed = "
						+ speedBGA[t][i] + ".");
			}
	}
	...
	// =============================================
	{
		for (int t = 0; t < NUM_STATES; t++) {
			...
			for (int i = 0; i < 2; i++) {
				double[] ret = processTransition(angleSARJ[t][i],
						speedSARJ[t][i], angleSARJ[tt][i], speedSARJ[tt][i],
			...
			for (int i = 0; i < 8; i++) {
				double[] ret = processTransition(angleBGA[t][i],
						speedBGA[t][i], angleBGA[tt][i], speedBGA[tt][i],
						MAX_BGA_SPEED, MAX_BGA_ACC);
				if (ret[0] != 1.0) {
	...
	// =============================================
	{
		double[] inp = new double[] { t, alpha, beta, yaw, angleSARJ[t][0],
				angleSARJ[t][1], angleBGA[t][0], angleBGA[t][1],
				angleBGA[t][2], angleBGA[t][3], angleBGA[t][4], angleBGA[t][5],
	...
	// =============================================
	{
		for (int id = 0; id < 8; id++) {
			...
			int counter = 0;
			// simulate 100 rotations around the orbit, since it works quick
			// anyway
			boolean ok = true;
			for (int x = 0; x < 100; x++) {
				for (int t = 0; t < NUM_STATES; t++) {
	...
	// =============================================
	double[] processTransition(double angle1, double speed1, double angle2,
			double speed2, double maxSpeed, double maxAcc)
	// =============================================
	{
		// make sure speed1 >= 0, to reduce the number of cases
		if (speed1 < 0)
			return processTransition(-angle1, -speed1, -angle2, -speed2,
					maxSpeed, maxAcc);
		...
		if (!canMakeTransition(shift, speed1, speed2, -maxSpeed, maxSpeed,
				maxAcc))
			return res;
	...
	// =============================================
	boolean canMakeTransition(double shift, double speed1, double speed2,
			double minSpeed, double maxSpeed, double maxAcc)
	// =============================================
	{
		...
		if (t1 <= t2) {
			...
			double t = (speed1 - speed2 + TIME_PER_STATE * maxAcc) / 2.0
					/ maxAcc;
		...
		if (t1 <= t2) {
			...
			double t = (speed2 - speed1 + TIME_PER_STATE * maxAcc) / 2.0
					/ maxAcc;
	...
	// longerons (first NUM_LONGERONS values for 1A, then for 2A, then for 3A
	// and so on).
	// =============================================
	...
	// =============================================
	{
		...
		ISSVis.sunTransform = ISSVis.makeSunProjection(ISSVis.beta,
				ISSVis.alpha);
		ISSVis.inverse = ISSVis.makeInverseSunProjection(ISSVis.beta,
				ISSVis.alpha);
		ISSVis.toSun = new V();
	...
	// =============================================
	{
		...
		double score = power
				* Math.min(1.0, Math.pow(2.0, (80.0 - maxBGArotation) / 300));
...
/* *********************************************
 * DDDD RRRR AAA W W EEEEE RRRR D D R R A A W W E R R D D RRRR AAAAA W W W EEE
 * RRRR D D R R A A WW WW E R R DDDD R R A A W W EEEEE R R
 * *********************************************
 */
...
// *********************************************
{
	...
	/*
	 * DDDD RRRR AAA W W EEEEE RRRR K K EEEEE Y Y L III SSS TTTTT EEEEE N N
	 * TTTTT EEEEE RRRR D D R R A A W W E R R K K E Y Y L I S T E NN N T E R R D
	 * D RRRR AAAAA W W W EEE RRRR KK EEE Y L I SSS T EEE N N N T EEE RRRR D D R
	 * R A A WW WW E R R K K E Y L I S T E N NN T E R R DDDD R R A A W W EEEEE R
	 * R K K EEEEE Y LLLLL III SSS T EEEEE N N T EEEEE R R
	 */
	// *********************************************
	...
	/*
	 * DDDD RRRR AAA W W EEEEE RRRR PPPP AAA N N EEEEE L D D R R A A W W E R R P
	 * P A A NN N E L D D RRRR AAAAA W W W EEE RRRR PPPP AAAAA N N N EEE L D D R
	 * R A A WW WW E R R P A A N NN E L DDDD R R A A W W EEEEE R R P A A N N
	 * EEEEE LLLLL
	 */
	// *********************************************
	...
	// *********************************************
	{
		public void paint(Graphics g) {
			synchronized (paintMutex) {
				...
				BufferedImage bi = new BufferedImage(xz, yz,
				...
				for (int line = 0; line < titles[curFrame].length - 1; ++line) {
					g.drawChars(titles[curFrame][line].toCharArray(), 0,
							titles[curFrame][line].length(), xz / 2,
							20 + 20 * line);
				}
				if (titles[curFrame].length > 0)
					g.drawChars(titles[curFrame][titles[curFrame].length - 1]
							.toCharArray(), 0,
							titles[curFrame][titles[curFrame].length - 1]
									.length(), xz / 2, yz - 11);

				if (footer != null) {
					for (int line = 0; line < footer.length; ++line) {
						g.drawChars(footer[line].toCharArray(), 0,
								footer[line].length(), xz / 2, yz
										- (31 + 20 * line));
					}
				}
...
/************************************************************
 * BBBB BBBB OOO X X B B B B O O X X BBBB BBBB O O X B B B B O O X X BBBB BBBB
 * OOO X X
 ************************************************************/
...
// ************************************************************
{
	...
	// ************************************************************
	{
		...
		bound[0] = new Sphere(new P((b.xmin + b.xmax) / 2,
				(b.ymin + b.ymax) / 2, (b.zmin + b.zmax) / 2),
...
/************************************************************
 * III SSS SSS RRRR EEEEE AAA DDDD EEEEE RRRR I S S R R E A A D D E R R I SSS
 * SSS RRRR EEE AAAAA D D EEE RRRR I S S R R E A A D D E R R III SSS SSS ___ R R
 * EEEEE A A DDDD EEEEE R R
 ************************************************************/
...
// ************************************************************
{
	...
	// ************************************************************
	{
		...
		try {
			...
			System.err.println("number format exception (" + s + ") at line "
					+ linenumber);
...
/************************************************************
 * SSS TTTTT RRRR U U CCC TTTTT U U RRRR EEEEE S T R R U U C T U U R R E SSS T
 * RRRR U U C T U U RRRR EEE S T R R U U C T U U R R E SSS T R R UUU CCC T UUU R
 * R EEEEE
 ************************************************************/
...
// ************************************************************
{
	...
	// ************************************************************
	{
		...
		bb = new Prim[1];
		bb[0] = new Sphere(new P((b.xmin + b.xmax) / 2, (b.ymin + b.ymax) / 2,
				(b.zmin + b.zmax) / 2), 0.5 * Math.sqrt((b.xmin - b.xmax)
				* (b.xmin - b.xmax) + (b.ymin - b.ymax) * (b.ymin - b.ymax)
				+ (b.zmin - b.zmax) * (b.zmin - b.zmax)));
	}
...
/************************************************************
 * PPPP RRRR III M M L III SSS TTTTT P P R R I MM MM L I S T PPPP RRRR I M M M L
 * I SSS T P R R I M M L I S T P R R III M M LLLLL III SSS T
 ************************************************************/
// ************************************************************
class PrimList
// ************************************************************
...
/************************************************************
 * SSS PPPP H H EEEEE RRRR EEEEE S P P H H E R R E SSS PPPP HHHHH EEE RRRR EEE S
 * P H H E R R E SSS P H H EEEEE R R EEEEE
 ************************************************************/
...
// ************************************************************
{
	...
	// ************************************************************
	{
		...
		double t = 0.5 * Math.sqrt(disc) + a * ray.d.x + b * ray.d.y + c
				* ray.d.z;
		if (t <= 0)
			return null;
...
/************************************************************
 * CCC Y Y L III N N DDDD EEEEE RRRR C Y Y L I NN N D D E R R C Y L I N N N D D
 * EEE RRRR C Y L I N NN D D E R R CCC Y LLLLL III N N DDDD EEEEE R R
 ************************************************************/
...
// ************************************************************
{
	...
	Cylinder(P p1, P p2, double radius) {
		...
		normal = new V();
		rad = radius;
		length = Math.sqrt((a1.x - a2.x) * (a1.x - a2.x) + (a1.y - a2.y)
				* (a1.y - a2.y) + (a1.z - a2.z) * (a1.z - a2.z));
		n = new V((a2.x - a1.x) / length, (a2.y - a1.y) / length, (a2.z - a1.z)
				/ length);
		D1 = -(a1.x * n.x + a1.y * n.y + a1.z * n.z);
		D2 = -(a2.x * n.x + a2.y * n.y + a2.z * n.z);
		visible = true;
	...
	{
		/*
		 * 1, Note the following handy fact from vector algebra: If you have a
		 * line passing through the origin and an arbitray point, P, then the
		 * vector cross product of (a unit vector in the direction of the line)
		 * and (a vector, V, from the origin to P) gives you a vector that is in
		 * the direction from P to the closest point to P that is on the line.
		 * The length of this vector is the distance from this closest point to
		 * P. (Actually you don't have to use the origin, any point on the line
		 * also works but the algebra is a little simpler using the origin).
		 * 
		 * 2, The defintion of a (mathematical) cylinder is all the points that
		 * are a given distance (radius) from a line.
		 * 
		 * 3, Combining 1 and 2 yields |U-V|^2 = r^2
		 * 
		 * 4, The expression for a point on a ray is (ray.o + ray.d * t ) where
		 * ray.o is the start (origin) of the ray and ray.d is the direction
		 * vector of the ray.
		 * 
		 * 5 Substitue 4 into 3 giving |U - (ray.o + ray.d * t - O)|^2 = r^2
		 * This equation now represnts points of the ray that are also on the
		 * cylinder.
		 * 
		 * 6, Solve for t giving a quadratic equation in the variable t t^2 * a
		 * + t * b + c = 0
		 * 
		 * 7, if this equation has solutions that are real numbers then the ray
		 * hits the sphere and the points are (ray.o + ray.d * t1) and (ray.o +
		 * ray.d * t2) where t1 and t2 are the roots of the quadratic equation.
		 * 
		 * The equation has real solutions if the discriminate, (b^2 - 4 * a *
		 * c), is postive.
		 * 
		 * 
		 * So, procedure: If we translate the ray and cylinder by the same
		 * amount (vector) then the intersection points are also translated by
		 * this amount. So, translate ray and cylinder so that a point on the
		 * axis of the cylinder is at the origin (we know two points that are on
		 * the axis, a1 and a2, so this is easy.
		 * 
		 * Solve for the roots t1 and t2 and substitute back in to the original
		 * ray expression to give the intersection points.
		 * 
		 * Since we are dealing with a finite cylinder with circular end caps,
		 * we must check that the points are between the end caps.
		 * 
		 * Also we must test the ray for intersection with end caps. This is
		 * done by intersecting the ray with the plane of the end cap and then
		 * testing if the distance from the intersection point to the cap center
		 * is less than the radius.
		 * 
		 * If any of the intersection points above are valid then they are also
		 * tested to be sure that they are in the positive t direction from the
		 * ray origin (otherwise the intersection is on the line of the ray, but
		 * not on the half of the line which is the ray).
		 * 
		 * If any intersection points are still valid, then the closest one to
		 * the ray origin is the winner. In order to do shading you need to know
		 * the normal vector to the surface at the closest intersection point.
		 * For cylinders this is easy, as it is either V/|V| (from 1 above) or
		 * the normal to one of the end caps.
		 * 
		 * 
		 * In traditional ray casting you repeat this procedure for every
		 * cylinder (or a similar procedure on other types of geometric
		 * primitives, such as polygons) that potentially intersect the ray and
		 * choose the closest one.
		 * 
		 * That is how you do it in traditional ray casting. For shadow testing
		 * you can to a slightly simplified test. You cast a ray from a point on
		 * the object to the light source. If you get any valid intersection in
		 * the cylinder calculation, you can stop and know you are in shadow.
		 * You do no have to calculate both intersection points. Actually you
		 * don't even have to calculate the intersection point, all you have to
		 * do is know that there is a valid intersection with the cylinder
		 * somewhere. Once any primitive is found to intersect the ray, then you
		 * know you are in shadow and you do not have to test any more
		 * primitives for that ray.
		 */
		depth = 2e100;
		if (!visible)
			return null;
		...
		if (r_dot_n < 0.999999999 && r_dot_n > -0.999999999) { // ray not (anti)
																// parallel to
																// cylinder
			...
			if (t >= 0) {
				...
				i3 = t * ray.d.z + ray.o.z; // (i1, i2, i3) is intersection
											// point

				d = i1 * c1 + i2 * c2 + i3 * c3 + D1; // signed distance from
														// plane of end cap 1
				if (d >= 0 && d <= length) {
					...
					double prj = ((i1 - a1.x) * n.x + (i2 - a1.y) * n.y + (i3 - a1.z)
							* n.z);
		...
		// test end cap a1
		if (hits < 2) {
			if ((c1 * ray.d.x + c2 * ray.d.y + c3 * ray.d.z) != 0) { // ray not
																		// parallel
																		// to
																		// end
																		// cap
				...
				if (t >= 0) {
					...
					d = ((i1 - a1.x) * (i1 - a1.x) + (i2 - a1.y) * (i2 - a1.y) + (i3 - a1.z)
							* (i3 - a1.z));

					if (d <= rad * rad) {
						double dep = i1 * ray.d.x + i2 * ray.d.y + i3 * ray.d.z;
						if (dep < depth) {
							if (n.x * ISSVis.sray.d.x + n.y * ISSVis.sray.d.y
		...
		if (hits < 2) {
			if ((c1 * ray.d.x + c2 * ray.d.y + c3 * ray.d.z) != 0) {
				t = -((c1 * ray.o.x + c2 * ray.o.y + c3 * ray.o.z + D2) / (c1
						* ray.d.x + c2 * ray.d.y + c3 * ray.d.z));
				if (t >= 0) {
					double i1 = t * ray.d.x + ray.o.x;
					double i2 = t * ray.d.y + ray.o.y;
					double i3 = t * ray.d.z + ray.o.z;

					d = ((i1 - a2.x) * (i1 - a2.x) + (i2 - a2.y) * (i2 - a2.y) + (i3 - a2.z)
							* (i3 - a2.z));

					if (d <= rad * rad) {
						double dep = (i1) * ray.d.x + (i2) * ray.d.y + (i3)
								* ray.d.z;
						if (dep < depth) {
							if (n.x * ISSVis.sray.d.x + n.y * ISSVis.sray.d.y
									+ n.z * ISSVis.sray.d.z < 0) {
	...
	// ************************************************************
	{
		...
		if (r_dot_n < 0.999999999 && r_dot_n > -0.999999999) { // ray not (anti)
			...
			if (t >= 0) {
				...
				d = i1 * c1 + i2 * c2 + i3 * c3 + D1;
				if (d >= 0 && d <= length)
					return this; // farthest intersection is on finite cylinder
									// wall
			}
			...
			if (t >= 0) {
				...
				d = i1 * c1 + i2 * c2 + i3 * c3 + D1;
				if (d >= 0 && d <= length)
					return this; // nearest intersection is on finite cylinder
									// wall
			}
		}
...
/************************************************************
 * PPPP OOO L Y Y GGG OOO N N P P O O L Y Y G O O NN N PPPP O O L Y G GGG O O N
 * N N P O O L Y G G O O N NN P OOO LLLLL Y GGGG OOO N N
 ************************************************************/
...
// ************************************************************
{
	...
	void print() {
		System.out.print("Polygon plane " + A + "x + " + B + "y + " + C
				+ "z + " + D + " = 0");
		for (int i = 0; i < v.length; ++i) {
			v[i].print();
			System.out.print(" ");
	...
	void println() {
		System.out.println("Polygon plane " + A + "x + " + B + "y + " + C
				+ "z + " + D + " = 0");
		for (int i = 0; i < v.length; ++i) {
			v[i].print();
			System.out.print(" ");
	...
	// ************************************************************
	{
		...
		double t = -((A * ray.o.x + B * ray.o.y + C * ray.o.z + D) / (A
				* ray.d.x + B * ray.d.y + C * ray.d.z));
		if (t < 0)
		...
		for (int i = 0; i < v.length; i++) {
			int j = (i + 1) % v.length;
			double c1 = ((v[j].z - v[i].z) * (v[i].y - i2) - (v[j].y - v[i].y)
					* (v[i].z - i3));
			double c2 = ((v[j].x - v[i].x) * (v[i].z - i3) - (v[j].z - v[i].z)
					* (v[i].x - i1));
			double c3 = ((v[j].y - v[i].y) * (v[i].x - i1) - (v[j].x - v[i].x)
					* (v[i].y - i2));
	...
	// ************************************************************
	{
		A = (v[0].z - v[1].z) * (v[2].y - v[1].y) - (v[0].y - v[1].y)
				* (v[2].z - v[1].z);
		B = (v[0].x - v[1].x) * (v[2].z - v[1].z) - (v[0].z - v[1].z)
				* (v[2].x - v[1].x);
		C = (v[0].y - v[1].y) * (v[2].x - v[1].x) - (v[0].x - v[1].x)
				* (v[2].y - v[1].y);
...
// ************************************************************
/**************************************************************
 * TTTTT RRRR III AAA N N GGG L EEEEE T R R I A A NN N G L E T RRRR I AAAAA N N
 * N G GGG L EEE T R R I A A N NN G G L E T R R III A A N N GGGG LLLLL EEEEE
 *************************************************************/
// ************************************************************
class Triangle {
	// ************************************************************
...
/************************************************************
 * III SSS SSS V V III SSS I S S V V I S I SSS SSS V V I SSS I S S V V I S III
 * SSS SSS V III SSS
 ************************************************************/
...
// ************************************************************
{
	...
	// ************************************************************
	static void color_a_pixel(short[][] raster, int x, int y, Prim the_hit,
			Prim shadower)
	// ************************************************************
	{
		x *= 3;
	...
	// ************************************************************
	static void render_from_view(int res, Structure m, M view_proj,
			M inverse_view, V to_sun)
	// ************************************************************
	{
		...
		for (int iy = 0; iy < res; ++iy) // image coordinates
		{
			...
			for (int ix = 0; ix < res; ++ix) {
				...
				if (the_hit != null) {
					...
					double diffuse = (to_sun.x * the_hit.normal.x + to_sun.y
							* the_hit.normal.y + to_sun.z * the_hit.normal.z);
					if (diffuse < 0)
						diffuse = 0;
					...
					if (resample[iy][ix]) {
						...
						for (int jz = 0; jz < ss; ++jz) {
							for (int jx = 0; jx < ss; ++jx) {
								double z = zs + dz * (jz + rnd.nextDouble())
										/ ss;
								double x = xs + dx * (jx + rnd.nextDouble())
										/ ss;
								...
								if (the_hit != null) {
									sray.o.x = base.x + the_hit.depth
											* vray.d.x;
									sray.o.y = base.y + the_hit.depth
											* vray.d.y;
									sray.o.z = base.z + the_hit.depth
											* vray.d.z;
									the_hit.visible = false;
									shadower = m.closestIntersection(sray);
									the_hit.visible = true;
									double diffuse = (to_sun.x
											* the_hit.normal.x + to_sun.y
											* the_hit.normal.y + to_sun.z
											* the_hit.normal.z);
									if (diffuse < 0)
										diffuse = 0;
									diffuse = Math.sqrt(diffuse); // gamma
									the_hit.red = (short) ((ISSVis.i_diffuse
											* diffuse + ISSVis.i_ambient) * the_hit.basered);
									the_hit.green = (short) ((ISSVis.i_diffuse
											* diffuse + ISSVis.i_ambient) * the_hit.basegreen);
									the_hit.blue = (short) ((ISSVis.i_diffuse
											* diffuse + ISSVis.i_ambient) * the_hit.baseblue);
								}
								color_a_pixel(raster, ix, iy, the_hit, shadower);
								rd += raster[iy][ix * 3];
								gd += raster[iy][ix * 3 + 1];
	...
	{
		/*
		 * assumed structure of ISS model (ISS_simple.model) SAW = Solar Array
		 * Wing
		 * 
		 * m = whole ISS
		 * 
		 * m.child[0] = starboard truss with starboard SARJ m.child[1] = port
		 * truss with port SARJ
		 * 
	...
	// ************************************************************
	// ************************************************************
	static double binary_search(P lowin, P highin, P lowout, P highout,
			Prim prim)
	// ************************************************************
	{
		double hx, hy, hz;
	...
	// ************************************************************
	static int primCount(int level, int p1, int i1, int p2, int i2, int p3,
			int i3)
	// ************************************************************
	{
	...
	// ************************************************************
	static double subdivide_triangle2(int level, P v1, int s1, P v2, int s2,
			P v3, int s3, Prim prim, double area)
	// ************************************************************
	{
	...
	// ************************************************************
	static double subdivide_triangle1(int level, P v1, int p1, P v2, int p2,
			P v3, int p3, Prim prim, double area)
	// ************************************************************
	{
		// There are two cases,
	...
	// ************************************************************
	static int least(PrimList p1, int i1, PrimList p2, int i2, PrimList p3,
			int i3, PrimList p4, int i4)
	// ************************************************************
	{
		if (i1 >= p1.s)
			return 0;
	...
	// ************************************************************
	static double[] calculateOneBlanketT(Polygon panel, Structure m, V toSun,
			M proj, boolean is_panel_1)
	// ************************************************************
	{
		...
		for (int string = 0; string < 41; string++) {
			...
			for (int bw = 0; bw < 5; bw++) // width, blocks per string
			{
				...
				for (int bl = 0; bl < 2; ++bl) {
					...
					interp(ssbl, sstl, ((bl + 1) * block_l_step - half_l_gap),
							bktl);
					interp(ssbr, sstr, ((bl + 1) * block_l_step - half_l_gap),
							bktr);
					...
					shadow_fraction[string] += (t1 + t2); // factor of two
															// handled below
				}
			}
			shadow_fraction[string] *= 1.0 / 20.0; // 10 blocks/string x 2
													// tri/block
		}
	...
	// ************************************************************
	static double[] calculateFourLongerons(int side, int saw, Structure m,
			V toSun, M proj)
	// ************************************************************
	{
	...
	static void calculateOneSAW(int control_index, int side, int saw,
			Structure m, V toSun, M proj, double[] answer)
	// ************************************************************
	{
		double cosine = 0;
		...
		longeronShadowFraction = calculateFourLongerons(side, saw, m, toSun,
				proj);
	...
	// ************************************************************
	{
		...
		if (minute == 0) {
			...
			if (csv_cols == 22) {
				...
				System.out
						.println("CSV mode: relaxed (only very basic checks)");
				check_constraints = false;
			} else if (minute == 91) {
				System.err
						.println("ERROR: your CSV file needs to have 93 lines and have"
		...
		if (minute == 0 && check_constraints) {
			...
			if (beta != Double.parseDouble(col[0])) {
				System.err
						.println("ERROR: in strict mode, beta values in all rows"
								+ " must be the same. Minute = " + minute + ".");
				System.exit(0);
			}
			if (yaw != Double.parseDouble(col[1])) {
				System.err
						.println("ERROR: in strict mode, yaw values in all rows"
								+ " must be the same. Minute = " + minute + ".");
				System.exit(0);
			}
		} else {
		...
		if (check_constraints) {
			for (int c = 0; c < 10; c++) {
				// order 1A, 2A,
				position_history[minute][c] = Double
						.parseDouble(col[c * 2 + 2]);
				velocity_history[minute][c] = Double
						.parseDouble(col[c * 2 + 3]);
			}
			...
			for (int c = 0; c < 10; c++) {
				// order 1A, 2A,
				position_history[step_number][c] = Double
						.parseDouble(col[c + 3]);
	...
	// ************************************************************
	{
		if ((pat.startsWith(s[i]) || ("-" + pat).startsWith(s[i]) || ("--" + pat)
				.startsWith(s[i]))
				&& i < s.length - 1
				&& !s[i + 1].startsWith("-"))
			return true;
	...
	{
		if ((pat.startsWith(s[i]) || ("-" + pat).startsWith(s[i]) || ("--" + pat)
				.startsWith(s[i])) && i < s.length - 1) {
			try {
				double d = Double.parseDouble(s[i + 1]);
				return true;
			} catch (Exception e) {
				System.err.println("Double argument reqired for option " + s[i]
						+ " found: " + s[i + 1]);
				System.exit(3);
			}
		}
	 ...
	 * average_powers -- contains current average powers cold_minutes --
	 * contains current longeron failure counters isLibraryFrame -- if true,
	 * average_powers and cold_minutes are not updated, as well as title[6] (bad
	 * min)
	 */
	static String[] calculateTitles(int frameNumber, double[] ans,
			double[] average_powers, int[] coldMinutes, boolean isLibraryFrame) {
		...
		for (int saw = 0; saw < 8; ++saw) {
			...
			for (int blanket = 0; blanket < 2; ++blanket)
				for (int string = 0; string < 41; ++string) {
					...
					SAWpower[saw] += cosine_factor * shadowFactor
							* stringPowerScale;
					if (shadowFactor == 0)
						darkStrings[saw]++;
					if (shadowFactor < 1.0)
	 ...
	 */
	static String[] calculateFooters(double[] average_powers,
	...
	// ************************************************************
	{
		...
		foot[2] = "Total Average Power: " + dfmt.format(average_total_power)
				+ " watts";
	...
	// ************************************************************
	{
		...
		for (int id = 0; id < 8; id++) {
			System.out.println("Average power by solar array "
					...
					+ dfmt.format(checker.totalPower[id]) + " watts.");
		...
		for (int id = 0; id < 8; id++) {
			System.out.println("Total rotation by BGA "
					+ checker.SOLAR_ARRAY_NAME[id] + " = "
					+ dfmt.format(checker.totalRotation(id)) + " degrees.");
		}
		for (int id = 0; id < 8; id++) {
			System.out.println("Maximum longeron danger count by BGA "
					+ checker.SOLAR_ARRAY_NAME[id] + " = " + maxHeat[id]);
		}
		checkForErrors(checker);
			...
			try {
				...
				for (int step = 0; step < max_frame; step++) {
					...
					for (int saw = 0; saw < 8; ++saw) {
						...
						for (int blanket = 0; blanket < 2; ++blanket)
							for (int string = 0; string < 41; ++string) {
								checker.stringShadow[step][saw][blanket * 41
										+ string] = ans[8 + saw * 41 * 2
										+ blanket * 41 + string];
							}
						for (int longeron = 0; longeron < 4; ++longeron) {
							checker.longeronShadow[step][saw][longeron] = ans[8
									+ 41 * 2 * 8 + saw * 4 + longeron];
						}
					}

					titles = calculateTitles(step, ans, average_powers,
							coldMinutes, false);

					if (rendering) {
						Drawer.getDrawer().setFrame(step, big_raster, titles,
								null, "Minute " + step + " beta = " + beta);
				...
				if (rendering)
					Drawer.getDrawer().setFrame(max_frame - 1, big_raster,
							titles, foot,
							"Minute " + (max_frame - 1) + " beta = " + beta);
				...
				if (check_constraints) {
					...
					System.out.println("Channel: 1A Average Power "
							+ (average_powers[s++] / (1000)) + " kilowatts");
	...
	// ************************************************************
	{
		...
		for (int t = 0; t < checker.NUM_STATES; t++) {
			while (true) {
				...
				if (what == SOLUTION_CALL) {
					...
					titles = calculateTitles(t, checker.lastAns,
							average_powers, coldMinutes, false);

					if (rendering) {
						Drawer.getDrawer().setFrame(t, big_raster, titles,
								null, "Minute " + t + " beta = " + beta);
					}
					...
					if (render_flag == 1 && rendering) {
						do_render(false);
						titles = calculateTitles(t, checker.lastAns,
								average_powers, coldMinutes, true);
						Drawer.getDrawer().setFrame(t, big_raster, titles,
								null, "Library call frame");
					}
				} else {
					System.out
							.println("ERROR: expected "
									+ SOLUTION_CALL
									+ " (indicating getStateAtMinute call) or "
									+ LIBRARY_CALL
									+ " (indicating \"library\" method call). Received "
									+ what + " instead.");
					System.exit(0);
				}
			}
		...
		if (rendering)
			Drawer.getDrawer().setFrame(checker.NUM_STATES - 1, big_raster,
					titles, foot,
					"Minute " + (checker.NUM_STATES - 1) + " beta = " + beta);
