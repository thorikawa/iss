...
class ConstraintsChecker {
    ...
    // =============================================
    {
        ...
        for (int i = 0; i < angleSARJ[t].length; i++)
            if (angleSARJ[t][i] < 0.0 || angleSARJ[t][i] >= 360.0) {
                errors.add("Each SARJ angle needs to be from 0.0 " + "(inclusive) to 360.0 (exclusive). Minute = " + t + ". SARJ = " + SARJ_NAME[i] + ". Your angle = " + angleSARJ[t][i] + ".");
            }

        for (int i = 0; i < angleBGA[t].length; i++)
            if (angleBGA[t][i] < 0.0 || angleBGA[t][i] >= 360.0) {
                errors.add("Each BGA angle needs to be from 0.0 " + "(inclusive) to 360.0 (exclusive). Minute = " + t + ". BGA = " + SOLAR_ARRAY_NAME[i] + ". Your angle = " + angleBGA[t][i] + ".");
            }

        for (int i = 0; i < speedSARJ[t].length; i++)
            if (Math.abs(speedSARJ[t][i]) > MAX_SARJ_SPEED) {
                errors.add("The absolute value of SARJ rotation speed " + "can't exceed " + MAX_SARJ_SPEED + ". Minute = " + t + ". SARJ = " + SARJ_NAME[i] + ". Your speed = " + speedSARJ[t][i] + ".");
            }

        for (int i = 0; i < speedBGA[t].length; i++)
            if (Math.abs(speedBGA[t][i]) > MAX_BGA_SPEED) {
                errors.add("The absolute value of BGA rotation speed " + " can't exceed " + MAX_BGA_SPEED + ". Minute = " + t + ". BGA = " + SOLAR_ARRAY_NAME[i] + ". Your speed = " + speedBGA[t][i]
                        + ".");
            }
    }
    ...
    // =============================================
    {
        for (int t = 0; t < NUM_STATES; t++) {
            ...
            for (int i = 0; i < 2; i++) {
                double[] ret = processTransition(angleSARJ[t][i], speedSARJ[t][i], angleSARJ[tt][i], speedSARJ[tt][i], MAX_SARJ_SPEED, MAX_SARJ_ACC);
                if (ret[0] != 1.0) {
                    errors.add("It is impossible to make a transition between" + " your SARJ configurations from minute " + t + " to minute " + tt + ". SARJ = " + SARJ_NAME[i] + ".");
                }
                rotSARJ[t][i] = ret[1];
            }

            for (int i = 0; i < 8; i++) {
                double[] ret = processTransition(angleBGA[t][i], speedBGA[t][i], angleBGA[tt][i], speedBGA[tt][i], MAX_BGA_SPEED, MAX_BGA_ACC);
                if (ret[0] != 1.0) {
                    errors.add("It is impossible to make a transition" + " between your BGA configurations from minute " + t + " to minute " + tt + ". BGA = " + SOLAR_ARRAY_NAME[i] + ".");
                }
                rotBGA[t][i] = ret[1];
            }
        }
    ...
    // =============================================
    {
        double[] inp = new double[] {t, alpha, beta, yaw, angleSARJ[t][0], angleSARJ[t][1], angleBGA[t][0], angleBGA[t][1], angleBGA[t][2], angleBGA[t][3], angleBGA[t][4], angleBGA[t][5],
                angleBGA[t][6], angleBGA[t][7] };
...
/* *********************************************
 * DDDD RRRR AAA W W EEEEE RRRR D D R R A A W W E R R D D RRRR AAAAA W W W EEE RRRR D D R R A A WW WW E R R DDDD R R A A W W EEEEE R R*********************************************
 */
...
// *********************************************
{
    ...
    /*
     * DDDD RRRR AAA W W EEEEE RRRR K K EEEEE Y Y L III SSS TTTTT EEEEE N N TTTTT EEEEE RRRR D D R R A A W W E R R K K E Y Y L I S T E NN N T E R R D D RRRR AAAAA W W W EEE RRRR KK EEE Y L I SSS T EEE
     * N N N T EEE RRRR D D R R A A WW WW E R R K K E Y L I S T E N NN T E R R DDDD R R A A W W EEEEE R R K K EEEEE Y LLLLL III SSS T EEEEE N N T EEEEE R R
     */
    // *********************************************
    ...
    /*
     * DDDD RRRR AAA W W EEEEE RRRR PPPP AAA N N EEEEE L D D R R A A W W E R R P P A A NN N E L D D RRRR AAAAA W W W EEE RRRR PPPP AAAAA N N N EEE L D D R R A A WW WW E R R P A A N NN E L DDDD R R A A
     * W W EEEEE R R P A A N N EEEEE LLLLL
     */
    // *********************************************
        ...
        public void paint(Graphics g) {
            synchronized (paintMutex) {
                ...
                if (titles[curFrame].length > 0)
                    g.drawChars(titles[curFrame][titles[curFrame].length - 1].toCharArray(), 0, titles[curFrame][titles[curFrame].length - 1].length(), xz / 2, yz - 11);
...
// ************************************************************
{
    ...
    // ************************************************************
    {
        ...
        bound(b);
        bound = new Prim[1];
        bound[0] = new Sphere(new P((b.xmin + b.xmax) / 2, (b.ymin + b.ymax) / 2, (b.zmin + b.zmax) / 2), 0.5 * Math.sqrt((b.xmin - b.xmax) * (b.xmin - b.xmax) + (b.ymin - b.ymax) * (b.ymin - b.ymax)
...
/************************************************************
 * III SSS SSS RRRR EEEEE AAA DDDD EEEEE RRRR I S S R R E A A D D E R R I SSS SSS RRRR EEE AAAAA D D EEE RRRR I S S R R E A A D D E R R III SSS SSS ___ R R EEEEE A A DDDD EEEEE R R
 ************************************************************/
...
/************************************************************
 * SSS TTTTT RRRR U U CCC TTTTT U U RRRR EEEEE S T R R U U C T U U R R E SSS T RRRR U U C T U U RRRR EEE S T R R U U C T U U R R E SSS T R R UUU CCC T UUU R R EEEEE
 ************************************************************/
...
// ************************************************************
{
    ...
    // ************************************************************
    {
        ...
        bb = new Prim[1];
        bb[0] = new Sphere(new P((b.xmin + b.xmax) / 2, (b.ymin + b.ymax) / 2, (b.zmin + b.zmax) / 2), 0.5 * Math.sqrt((b.xmin - b.xmax) * (b.xmin - b.xmax) + (b.ymin - b.ymax) * (b.ymin - b.ymax)
                + (b.zmin - b.zmax) * (b.zmin - b.zmax)));
    }
...
/************************************************************
 * PPPP RRRR III M M L III SSS TTTTT P P R R I MM MM L I S T PPPP RRRR I M M M L I SSS T P R R I M M L I S T P R R III M M LLLLL III SSS T
 ************************************************************/
// ************************************************************
class PrimList
// ************************************************************
...
/************************************************************
 * CCC Y Y L III N N DDDD EEEEE RRRR C Y Y L I NN N D D E R R C Y L I N N N D D EEE RRRR C Y L I N NN D D E R R CCC Y LLLLL III N N DDDD EEEEE R R
 ************************************************************/
...
// ************************************************************
{
    ...
    {
        /*
         * 1, Note the following handy fact from vector algebra: If you have a line passing through the origin and an arbitray point, P, then the vector cross product of (a unit vector in the
         * direction of the line) and (a vector, V, from the origin to P) gives you a vector that is in the direction from P to the closest point to P that is on the line. The length of this vector is
         * the distance from this closest point to P. (Actually you don't have to use the origin, any point on the line also works but the algebra is a little simpler using the origin). 2, The
         * defintion of a (mathematical) cylinder is all the points that are a given distance (radius) from a line. 3, Combining 1 and 2 yields |U-V|^2 = r^2 4, The expression for a point on a ray is
         * (ray.o + ray.d * t ) where ray.o is the start (origin) of the ray and ray.d is the direction vector of the ray. 5 Substitue 4 into 3 giving |U - (ray.o + ray.d * t - O)|^2 = r^2 This
         * equation now represnts points of the ray that are also on the cylinder. 6, Solve for t giving a quadratic equation in the variable t t^2 * a + t * b + c = 0 7, if this equation has
         * solutions that are real numbers then the ray hits the sphere and the points are (ray.o + ray.d * t1) and (ray.o + ray.d * t2) where t1 and t2 are the roots of the quadratic equation. The
         * equation has real solutions if the discriminate, (b^2 - 4 * a * c), is postive. So, procedure: If we translate the ray and cylinder by the same amount (vector) then the intersection points
         * are also translated by this amount. So, translate ray and cylinder so that a point on the axis of the cylinder is at the origin (we know two points that are on the axis, a1 and a2, so this
         * is easy. Solve for the roots t1 and t2 and substitute back in to the original ray expression to give the intersection points. Since we are dealing with a finite cylinder with circular end
         * caps, we must check that the points are between the end caps. Also we must test the ray for intersection with end caps. This is done by intersecting the ray with the plane of the end cap
         * and then testing if the distance from the intersection point to the cap center is less than the radius. If any of the intersection points above are valid then they are also tested to be
         * sure that they are in the positive t direction from the ray origin (otherwise the intersection is on the line of the ray, but not on the half of the line which is the ray). If any
         * intersection points are still valid, then the closest one to the ray origin is the winner. In order to do shading you need to know the normal vector to the surface at the closest
         * intersection point. For cylinders this is easy, as it is either V/|V| (from 1 above) or the normal to one of the end caps. In traditional ray casting you repeat this procedure for every
         * cylinder (or a similar procedure on other types of geometric primitives, such as polygons) that potentially intersect the ray and choose the closest one. That is how you do it in
         * traditional ray casting. For shadow testing you can to a slightly simplified test. You cast a ray from a point on the object to the light source. If you get any valid intersection in the
         * cylinder calculation, you can stop and know you are in shadow. You do no have to calculate both intersection points. Actually you don't even have to calculate the intersection point, all
         * you have to do is know that there is a valid intersection with the cylinder somewhere. Once any primitive is found to intersect the ray, then you know you are in shadow and you do not have
        ...
        if (r_dot_n < 0.999999999 && r_dot_n > -0.999999999) { // ray not (anti) parallel to cylinder
            ...
            if (t >= 0) {
                ...
                if (d >= 0 && d <= length) {
                    ...
                    double prj = ((i1 - a1.x) * n.x + (i2 - a1.y) * n.y + (i3 - a1.z) * n.z);
                    ...
                    double ny = i2 - ay;
                    double nz = i3 - az;
                    double iv = 1.0 / Math.sqrt(nx * nx + ny * ny + nz * nz);
                    nx *= iv;
                    ny *= iv;
                    nz *= iv;
        ...
        // test end cap a1
        if (hits < 2) {
            if ((c1 * ray.d.x + c2 * ray.d.y + c3 * ray.d.z) != 0) { // ray not parallel to end cap
                t = -(c1 * ray.o.x + c2 * ray.o.y + c3 * ray.o.z + D1) / (c1 * ray.d.x + c2 * ray.d.y + c3 * ray.d.z);
                if (t >= 0) {
                    ...
                    if (d <= rad * rad) {
                        double dep = i1 * ray.d.x + i2 * ray.d.y + i3 * ray.d.z;
                        if (dep < depth) {
                            if (n.x * ISSVis.sray.d.x + n.y * ISSVis.sray.d.y + n.z * ISSVis.sray.d.z > 0) {
                                normal.x = n.x;
                                normal.y = n.y;
                                normal.z = n.z;
                            } else {
        ...
        if (hits < 2) {
            if ((c1 * ray.d.x + c2 * ray.d.y + c3 * ray.d.z) != 0) {
                t = -((c1 * ray.o.x + c2 * ray.o.y + c3 * ray.o.z + D2) / (c1 * ray.d.x + c2 * ray.d.y + c3 * ray.d.z));
                if (t >= 0) {
                    double i1 = t * ray.d.x + ray.o.x;
                    double i2 = t * ray.d.y + ray.o.y;
                    double i3 = t * ray.d.z + ray.o.z;

                    d = ((i1 - a2.x) * (i1 - a2.x) + (i2 - a2.y) * (i2 - a2.y) + (i3 - a2.z) * (i3 - a2.z));
...
/************************************************************
 * PPPP OOO L Y Y GGG OOO N N P P O O L Y Y G O O NN N PPPP O O L Y G GGG O O N N N P O O L Y G G O O N NN P OOO LLLLL Y GGGG OOO N N
 ************************************************************/
...
// ************************************************************
{
    ...
    void print() {
        System.out.print("Polygon plane " + A + "x + " + B + "y + " + C + "z + " + D + " = 0");
        for (int i = 0; i < v.length; ++i) {
            v[i].print();
            System.out.print(" ");
        }
    ...
    void println() {
        System.out.println("Polygon plane " + A + "x + " + B + "y + " + C + "z + " + D + " = 0");
        for (int i = 0; i < v.length; ++i) {
            v[i].print();
            System.out.print(" ");
        }
    ...
    // ************************************************************
    {
        ...
        for (int i = 0; i < v.length; i++) {
            int j = (i + 1) % v.length;
            double c1 = ((v[j].z - v[i].z) * (v[i].y - i2) - (v[j].y - v[i].y) * (v[i].z - i3));
            double c2 = ((v[j].x - v[i].x) * (v[i].z - i3) - (v[j].z - v[i].z) * (v[i].x - i1));
            double c3 = ((v[j].y - v[i].y) * (v[i].x - i1) - (v[j].x - v[i].x) * (v[i].y - i2));
...
// ************************************************************
/**************************************************************
 * TTTTT RRRR III AAA N N GGG L EEEEE T R R I A A NN N G L E T RRRR I AAAAA N N N G GGG L EEE T R R I A A N NN G G L E T R R III A A N N GGGG LLLLL EEEEE
 *************************************************************/
// ************************************************************
class Triangle {
    // ************************************************************
...
/************************************************************
 * III SSS SSS V V III SSS I S S V V I S I SSS SSS V V I SSS I S S V V I S III SSS SSS V III SSS
 ************************************************************/
...
// ************************************************************
{
        ...
        while (Drawer.getDrawer() != null) {
            if (Drawer.getDrawer().animationMode) {
                ...
                synchronized (Drawer.getDrawer().paintMutex) {
                    i = (Drawer.getDrawer().curFrame + 1) % Drawer.getDrawer().frameCnt;
                }
    ...
    // ************************************************************
    static void color_a_pixel(short[][] raster, int x, int y, Prim the_hit, Prim shadower)
    // ************************************************************
    {
        x *= 3;
        if (the_hit == null) {
        ...
        for (int iy = 0; iy < res; ++iy) // image coordinates
        {
            ...
            for (int ix = 0; ix < res; ++ix) {
                ...
                if (the_hit != null) {
                    ...
                    double diffuse = (to_sun.x * the_hit.normal.x + to_sun.y * the_hit.normal.y + to_sun.z * the_hit.normal.z);
                    if (diffuse < 0)
                        diffuse = 0;
                    diffuse = Math.sqrt(diffuse); // gamma
                    the_hit.red = (short) ((ISSVis.i_diffuse * diffuse + ISSVis.i_ambient) * the_hit.basered);
                    the_hit.green = (short) ((ISSVis.i_diffuse * diffuse + ISSVis.i_ambient) * the_hit.basegreen);
                    the_hit.blue = (short) ((ISSVis.i_diffuse * diffuse + ISSVis.i_ambient) * the_hit.baseblue);
                }
        ...
        if (ss > 1) {
            for (int iy = 1; iy < res - 1; ++iy) {
                for (int ix = 1; ix < res - 1; ++ix) {
                    if (hitter[iy][ix] != hitter[iy][ix + 1] || hitter[iy][ix] != hitter[iy][ix - 1] || hitter[iy][ix] != hitter[iy + 1][ix] || hitter[iy][ix] != hitter[iy - 1][ix] ||

                    raster[iy][ix * 3 - 3] != raster[iy][ix * 3 + 0] || raster[iy][ix * 3 - 2] != raster[iy][ix * 3 + 1] || raster[iy][ix * 3 - 1] != raster[iy][ix * 3 + 2]
                            || raster[iy][ix * 3 + 0] != raster[iy][ix * 3 + 3] || raster[iy][ix * 3 + 1] != raster[iy][ix * 3 + 4] || raster[iy][ix * 3 + 2] != raster[iy][ix * 3 + 5] ||

                            raster[iy - 1][ix * 3 + 0] != raster[iy][ix * 3 + 0] || raster[iy - 1][ix * 3 + 1] != raster[iy][ix * 3 + 1] || raster[iy - 1][ix * 3 + 2] != raster[iy][ix * 3 + 2]
                            || raster[iy][ix * 3 + 0] != raster[iy + 1][ix * 3 + 0] || raster[iy][ix * 3 + 1] != raster[iy + 1][ix * 3 + 1] || raster[iy][ix * 3 + 2] != raster[iy + 1][ix * 3 + 2]) {
                        resample[iy][ix] = true;
        ...
        for (int iy = 0; iy < res; ++iy) // image coordinates
        {
            ...
            for (int ix = 0; ix < res; ++ix) {
                ...
                if (the_hit == null) {
                    ...
                    raster[iy][++jx] = (short) ((ISSVis.i_diffuse * diffuse + ISSVis.i_ambient) * the_hit.basered);
                    raster[iy][++jx] = (short) ((ISSVis.i_diffuse * diffuse + ISSVis.i_ambient) * the_hit.basegreen);
                    raster[iy][++jx] = (short) ((ISSVis.i_diffuse * diffuse + ISSVis.i_ambient) * the_hit.baseblue);
                }
            }
        }
        int ac = 0;
                        ...
                        for (int jz = 0; jz < ss; ++jz) {
                            for (int jx = 0; jx < ss; ++jx) {
                                ...
                                if (sub_hit == null) {
                                    ...
                                    rd += (short) ((ISSVis.i_diffuse * diffuse + ISSVis.i_ambient) * sub_hit.basered);
                                    gd += (short) ((ISSVis.i_diffuse * diffuse + ISSVis.i_ambient) * sub_hit.basegreen);
                                    bd += (short) ((ISSVis.i_diffuse * diffuse + ISSVis.i_ambient) * sub_hit.baseblue);
                                }
                            }
                        }
                        raster[iy][ix * 3] = (short) (rd / (ss * ss) + 0.5);
    ...
    {
        /*
         * assumed structure of ISS model (ISS_simple.model) SAW = Solar Array Wing m = whole ISS m.child[0] = starboard truss with starboard SARJ m.child[1] = port truss with port SARJ
         * m.child[0].child[0] is first SAW in starboard truss with BGA 1A m.child[0].child[1] is second SAW in starboard truss with BGA 3A m.child[0].child[2] is third SAW in starboard truss with BGA
         * 1B m.child[0].child[3] is fourth SAW in starboard truss with BGA 3B m.child[1].child[0] is first SAW in port truss with BGA 2A m.child[1].child[1] is second SAW in port truss with BGA 4A
         * m.child[1].child[2] is third SAW in port truss with BGA 2B m.child[1].child[3] is fourth SAW in port truss with BGA 4B m.child[].child[].solid[2] is one panel
         * m.child[].child[].solid[2].obj[0] is the polygon m.child[].child[].solid[3] is the other panel m.child[].child[].solid[3].obj[0] is the polygon m.child[].child[].solid[4] is the longerons
        ...
        for (int s = 0; s < 10; s++)
            ang[s] = (ang[s] - rotation_angle_offset[s]) * rotation_direction[s];
    ...
    // ************************************************************
    // ************************************************************
    static double binary_search(P lowin, P highin, P lowout, P highout, Prim prim)
    // ************************************************************
    {
        double hx, hy, hz;
        double lx, ly, lz;
    ...
    // ************************************************************
    static int primCount(int level, int p1, int i1, int p2, int i2, int p3, int i3)
    // ************************************************************
    {
        ...
        if (i2 < p[p2].s && p[p2].valid[i2]) {
            if (p[p2].prim[i2].order < p[p1].prim[i1].order)
                return 0;
            else if (p[p2].prim[i2].order == p[p1].prim[i1].order)
                c = 2;
        }
        if (i3 < p[p3].s && p[p3].valid[i3]) {
            if (p[p3].prim[i3].order < p[p1].prim[i1].order)
                return 0;
            else if (p[p3].prim[i3].order == p[p1].prim[i1].order)
                c++;
        }
        return c;
    }
    ...
    // ************************************************************
    {
        ...
        while (p[1].s > s1 || p[2].s > s2 || p[0].s > s0) {
            if (primCount(level, 1, s1, 2, s2, 0, s0) == 2 && primCount(level, 2, s2, 1, s1, 0, s0) == 2) {
                a = subdivide_triangle2(level, t.v2, 1, t.v3, 2, t.v1, 0, p[1].prim[s1], area);
                if (a > 0)
                    return a;
                p[1].valid[s1] = false;
                p[2].valid[s2] = false;
            } else if (primCount(level, 2, s2, 1, s1, 0, s0) == 2 && primCount(level, 0, s0, 2, s2, 1, s1) == 2) {
                a = subdivide_triangle2(level, t.v1, 0, t.v3, 2, t.v2, 1, p[0].prim[s0], area);
                if (a > 0)
                    return a;
                p[0].valid[s0] = false;
                p[2].valid[s2] = false;
            } else if (primCount(level, 1, s1, 2, s2, 0, s0) == 2 && primCount(level, 0, s0, 2, s2, 1, s1) == 2) {
                a = subdivide_triangle2(level, t.v1, 0, t.v2, 1, t.v3, 2, p[1].prim[s1], area);
                if (a > 0)
                    return a;
                p[1].valid[s1] = false;
                p[0].valid[s0] = false;
        ...
        while (p[1].s > s1 || p[2].s > s2 || p[0].s > s0) {
            if (primCount(level, 1, s1, 2, s2, 0, s0) > 0) {
                a = subdivide_triangle1(level, t.v2, 1, t.v1, 0, t.v3, 2, p[1].prim[s1], area);
                if (a > 0)
                    return a;
            } else if (primCount(level, 2, s2, 1, s1, 0, s0) > 0) {
                a = subdivide_triangle1(level, t.v3, 2, t.v2, 1, t.v1, 0, p[2].prim[s2], area);
                if (a > 0)
                    return a;
            } else if (primCount(level, 0, s0, 2, s2, 1, s1) > 0) {
                a = subdivide_triangle1(level, t.v1, 0, t.v2, 1, t.v3, 2, p[0].prim[s0], area);
                if (a > 0)
                    return a;
            }
    ...
    // ************************************************************
    static double subdivide_triangle2(int level, P v1, int s1, P v2, int s2, P v3, int s3, Prim prim, double area)
    // ************************************************************
    {
        ...
        if (a13 > 1e-3 || a23 > 1e-3) // case 1 and case 2
        {
            ...
            if (level < temp_max_level - 1) {
                ...
                t2.caseno = 21;
                double ans = covered_area + subdivide_triangle(level + 1, area - covered_area);
    ...
    // ************************************************************
    static double subdivide_triangle1(int level, P v1, int p1, P v2, int p2, P v3, int p3, Prim prim, double area)
    // ************************************************************
    {
        // There are two cases,
    ...
    // ************************************************************
    static int least(PrimList p1, int i1, PrimList p2, int i2, PrimList p3, int i3, PrimList p4, int i4)
    // ************************************************************
    {
        if (i1 >= p1.s)
            return 0;
        ...
        if (i2 < p2.s && p2.prim[i2].order < od)
            return 0;
        if (i3 < p3.s && p3.prim[i3].order < od)
            return 0;
        if (i4 < p4.s && p4.prim[i4].order < od)
            return 0;
    ...
    // ************************************************************
    {
        ...
        if (minute == 0) {
            ...
            if (csv_cols == 22) {
                ...
                System.err.println("ERROR: your CSV file needs to have 93 lines and have" + "22 or 13 columns \non each line, except possibly " + "the first (did you forget that first line?)");
                System.exit(0);
            } else {
        ...
        if (minute == 0 && check_constraints) {
            ...
            if (beta != Double.parseDouble(col[0])) {
                System.err.println("ERROR: in strict mode, beta values in all rows" + " must be the same. Minute = " + minute + ".");
                System.exit(0);
            }
            if (yaw != Double.parseDouble(col[1])) {
                System.err.println("ERROR: in strict mode, yaw values in all rows" + " must be the same. Minute = " + minute + ".");
                System.exit(0);
            }
        } else {
    ...
    // ************************************************************
    static void colorit(Structure m, int sarj, int bga, int r, int g, int b)
    // ************************************************************
    {
    ...
    // ************************************************************
    ISSVis(String[] tokens, boolean render1, boolean render92, int rez)
    // ************************************************************
    {
    ...
    // ************************************************************
    {
        ...
        titles[0] = "SAW:            1A       2A       3A       4A" + "       1B       2B       3B       4B";
    ...
    // ************************************************************
    /*
     * ans -- return value from calculateOnePosition (or evaluateSingleState) average_powers -- contains current average powers cold_minutes -- contains current longeron failure counters
    ...
    static String[] calculateTitles(int frameNumber, double[] ans, double[] average_powers, int[] coldMinutes, boolean isLibraryFrame) {
        ...
        for (int saw = 0; saw < 8; ++saw) {
            ...
            for (int blanket = 0; blanket < 2; ++blanket)
                for (int string = 0; string < 41; ++string) {
                    double f = ans[8 + saw * 41 * 2 + blanket * 41 + string];
                    double shadowFactor = Math.max(0.0, 1 - f * 5);

                    double cosine_factor = Math.max(0.0, ans[saw]);
                    SAWpower[saw] += cosine_factor * shadowFactor * stringPowerScale;
                    if (shadowFactor == 0)
                        darkStrings[saw]++;
                    if (shadowFactor < 1.0)
                        partialStrings[saw]++;
            ...
            if (!isLibraryFrame)
                average_powers[saw] += SAWpower[saw];
    ...
    // ************************************************************
    {
        ...
        for (int pwr = 0; pwr < 8; pwr++)
            foot[0] += fw((average_powers[pwr] - checker.MIN_POWER_REQ[pwr]) / 1000.0) + " ";

        foot[1] = "Average Powers: ";
        for (int pwr = 0; pwr < 8; pwr++)
            foot[1] += fw(average_powers[pwr] / 1000.0) + " ";

        foot[2] = "Total Average Power: " + dfmt.format(average_total_power) + " watts";

        foot[3] = "";

        if (checker != null)
            foot[3] = "Score: " + dfmt.format(checker.rawScore());
    ...
    // ************************************************************
    static void do_final_prints(ConstraintsChecker checker, int[] maxHeat)
    // ************************************************************
    {
        double avetotal = 0.0;
        for (int id = 0; id < 8; id++) {
            System.out.println("Average power by solar array " + checker.SOLAR_ARRAY_NAME[id] + " = " + dfmt.format(checker.totalPower[id]) + " watts.");
            avetotal += checker.totalPower[id];
        }
        System.out.println("Total power = " + dfmt.format(avetotal));
        System.out.println("Number of underpowered solar arrays (M) = " + checker.lowPowerBGAs.size());
        for (int id = 0; id < 8; id++) {
            System.out.println("Total rotation by BGA " + checker.SOLAR_ARRAY_NAME[id] + " = " + dfmt.format(checker.totalRotation(id)) + " degrees.");
        }
        for (int id = 0; id < 8; id++) {
            System.out.println("Maximum longeron danger count by BGA " + checker.SOLAR_ARRAY_NAME[id] + " = " + maxHeat[id]);
        }
        checkForErrors(checker);
            ...
            try {
                ...
                for (int step = 0; step < max_frame; step++) {
                    ...
                    for (int saw = 0; saw < 8; ++saw) {
                        ...
                        for (int blanket = 0; blanket < 2; ++blanket)
                            for (int string = 0; string < 41; ++string) {
                                checker.stringShadow[step][saw][blanket * 41 + string] = ans[8 + saw * 41 * 2 + blanket * 41 + string];
                            }
                        for (int longeron = 0; longeron < 4; ++longeron) {
                            checker.longeronShadow[step][saw][longeron] = ans[8 + 41 * 2 * 8 + saw * 4 + longeron];
                        }
                    }
                ...
                System.out.println("  Total average power: " + dfmt.format(tot_power) + " watts.");
                int s = 0;
                ...
                if (check_constraints) {
                    ...
                    s = 0;
                    System.out.println("Channel: 1A Average Power " + (average_powers[s++] / (1000)) + " kilowatts");
                    System.out.println("Channel: 2A Average Power " + (average_powers[s++] / (1000)) + " kilowatts");
                    System.out.println("Channel: 3A Average Power " + (average_powers[s++] / (1000)) + " kilowatts");
                    System.out.println("Channel: 4A Average Power " + (average_powers[s++] / (1000)) + " kilowatts");
                    System.out.println("Channel: 1B Average Power " + (average_powers[s++] / (1000)) + " kilowatts");
                    System.out.println("Channel: 2B Average Power " + (average_powers[s++] / (1000)) + " kilowatts");
                    System.out.println("Channel: 3B Average Power " + (average_powers[s++] / (1000)) + " kilowatts");
                    System.out.println("Channel: 4B Average Power " + (average_powers[s++] / (1000)) + " kilowatts");
                }
                if (rendering)
    ...
    // ************************************************************
    {
        ...
        for (int t = 0; t < checker.NUM_STATES; t++) {
            while (true) {
                ...
                if (what == SOLUTION_CALL) {
                    ...
                    for (int i = 0; i < 20; i++)
                        ret[i] = Double.parseDouble(scanner.next());
                    ...
                    System.out.println("ERROR: expected " + SOLUTION_CALL + " (indicating getStateAtMinute call) or " + LIBRARY_CALL + " (indicating \"library\" method call). Received " + what
                            + " instead.");
                    System.exit(0);
                }
            }
        ...
        String[] foot = calculateFooters(average_powers, (check_constraints ? checker : null));
        if (rendering)
            Drawer.getDrawer().setFrame(checker.NUM_STATES - 1, big_raster, titles, foot, "Minute " + (checker.NUM_STATES - 1) + " beta = " + beta);
    ...
    // ************************************************************
    {
        ...
        for (int a = 0; a < arg.length; a++) {
            if (match(arg[a], "exec")) {
                ...
                System.err.println("Error: unknown command option " + arg[a]);
                System.exit(77);
            }
        }
